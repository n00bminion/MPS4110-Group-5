...
meanDMedHP_Uni <-payGapData %>%
  ...
  pivot_wider(names_from = pre92, values_from = meanGap, names_prefix = "pre92_")%>%
  left_join(meanDMedHP_Yr,meanGap, by = "year")%>%
  rename(Overall_Mean_Gap = meanGap)%>%
...
# Facet grid of box plots of pay quartiles data ----
  ...
  geom_boxplot()+
  facet_grid(rows = vars(payGapMeasure),
            col = vars(year),
            labeller = label_both)+
  labs(y = "% Female")
