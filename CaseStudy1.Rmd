---
title: "Case Study 1"
author: "Zain Naseer/Kevin Stone/Minh Tran"
date: "2025-11-01"
output:
...
pay1 <- read_csv("data/UK Gender Pay Gap Data 2017.csv")
head(pay1)

pay2 <- read_csv("data/UK Gender Pay Gap Data 2018.csv")
head(pay2)

pay3 <- read_csv("data/UK Gender Pay Gap Data 2019.csv")
head(pay3)

pay4 <- read_csv("data/UK Gender Pay Gap Data 2020.csv")
head(pay4)

pay5 <- read_csv("data/UK Gender Pay Gap Data 2021.csv")
head(pay5)

pay6 <- read_csv("data/UK Gender Pay Gap Data 2022.csv")
head(pay6)

pay7 <- read_csv("data/UK Gender Pay Gap Data 2023.csv")
head(pay7)

pay8 <- read_csv("data/UK Gender Pay Gap Data 2024.csv")
head(pay8)
...
As mentioned above, any positive percentage difference means men’s pay is higher than women’s and vice versa. If we set men pay to £1, we can thus use the median percentage differences to calculate how much women are underpaid/overpaid in comparison for each £1 men are being paid for. Applying this to both hourly pay and bonus pay, we can see see the a relationship between the two pay types. 

First observation follows on the main points from table above, where we see a large number of data points clustered in the top right quadrant, indicating that men are generally paid more than women on both hourly basis and in term of bonus.

However, there are also a large number of women earning more bonuses than men, which is shown in the top right quadrant but the density is lower than top left quadrant. Interestingly, the women earn a signigficantly larger bonuses than men while still being paid less on an hourly basis.

Lastly, the bottom two quadrants show very few data points further support the point that women do not earn as men hourly.

Slicing the data by Years does not yield any meaningful results but splitting the data by Pre 1992 and Post 1992 institutions shows that the Post 1992 institutions have a wide spread of bonus pay differences compare to Pre 1992 institutions. 
...
ggplot(
  data = data,
  aes(x = DiffMedianBonusPercent / 100, y = DiffMedianHourlyPercent / 100, colour = as.factor(pre92))
) +
  geom_point() +
  xlim(-x_limit, x_limit) +
  ylim(-y_limit, y_limit) +
  annotate("rect", xmin = 0, xmax = -x_limit, ymin = 0, ymax = -x_limit, alpha = 0.2, fill = "red") +
  annotate("rect", xmin = 0, xmax = x_limit, ymin = 0, ymax = x_limit, alpha = 0.2, fill = "red") +
  annotate(
    "text",
  ...
  geom_xsideboxplot(
    aes(x = DiffMedianBonusPercent / 100, fill = as.factor(pre92)),
    orientation = "y",
    alpha = 0.5,
    outlier.shape = NA,
    show.legend = FALSE,
  ...
  geom_ysideboxplot(
    aes(y = DiffMedianHourlyPercent / 100, fill = as.factor(pre92)),
    alpha = 0.5,
    outlier.shape = NA,
    orientation = "x",
    show.legend = FALSE
...
The percentage of male and female in the top hourly pay quartile (highest paid) has seen a pay gap reduction since 2018, with it being at its lowest in 2025. In 2025 we have a median value of 52% for male representation in the Top Quartile, compared with a median of 48% for female representation. 

Apart from the Top Quartile, the other pay quartiles have a higher female representation. 
